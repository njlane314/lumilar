...
namespace BxDecay0Generator {
  PrimaryGeneratorActionMessenger::PrimaryGeneratorActionMessenger(PrimaryGeneratorAction * pga_) : G4UImessenger() {
    _pga_ = pga_;
...

    {
...

      {
        ...
        ssguidance << "the BxDecay0 double beta decay mode (supported values: [" << bxdecay0::DBDMODE_MIN << ',' << bxdecay0::DBDMODE_MAX << "])";
  ...
  G4String PrimaryGeneratorActionMessenger::GetCurrentValue(G4UIcommand * command_) {
    std::cerr << "[trace] BxDecay0Generator::PrimaryGeneratorActionMessenger::GetCurrentValue: Entering...\n";
    G4String curValue;
    ...
    std::cerr << "[log] BxDecay0Generator::PrimaryGeneratorActionMessenger::GetCurrentValue: curValue='" << curValue << "'\n";
    return curValue;
  }
  //_________________________________________________________________________________________
  void PrimaryGeneratorActionMessenger::SetNewValue(G4UIcommand * command_, G4String new_value_) {
    if (_pga_->IsTrace()) std::cerr << "[trace] BxDecay0Generator::PrimaryGeneratorActionMessenger::SetNewValue: Entering...\n";
    if (_pga_->IsDebug()) std::cerr << "[debug] BxDecay0Generator::PrimaryGeneratorActionMessenger::SetNewValue: "
                                    << "new_value='" << new_value_ << "'\n";

    if (command_ == _pga_gtor_apply_cmd_) {
      _pga_->ApplyConfiguration();  
      if (_pga_->IsDebug()) {
        std::cerr << "[debug] BxDecay0Generator::PrimaryGeneratorActionMessenger::SetNewValue: "
    ...
    if (command_ == _pga_gtor_mdl_cmd_) {
      ...
      if (not error) {
        ...
        if (_pga_->IsDebug()) {
          ...
          std::cerr << "[error] BxDecay0Generator::PrimaryGeneratorActionMessenger::SetNewValue: "
    ...
    if (_pga_->IsTrace()) std::cerr << "[trace] BxDecay0Generator::PrimaryGeneratorActionMessenger::SetNewValue: Exiting...\n";
    return;
  }
} 