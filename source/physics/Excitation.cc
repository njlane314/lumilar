...
std::pair<double, double> Excitation::CreateExcitation(const EnergyDeposit* energy_deposit, const Properties* material_properties) {    
    double expected_ionisations = energy_deposit->GetVisibleDeposit() / material_properties->loss_per_ionisation;
    ...
    double excitation_energy = energy_deposit->GetVisibleDeposit() - num_ionisations * (material_properties->ionisation_threshold + material_properties->thermal_loss);
