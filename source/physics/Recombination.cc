...
std::pair<double, double> Recombination::ProcessRecombination(const EnergyDeposit* energy_deposit, const Properties* material_properties, const std::pair<double, double> intrinsic_response) {
    double recombination_factor = ChargeRecombination(energy_deposit->GetStoppingPower(), material_properties->electric_field); 
...
//_________________________________________________________________________________________
double Recombination::ChargeRecombination(double dEdx, double electric_field) {
    return BirksRecombination(dEdx, electric_field) + EscapeRecombination(dEdx, electric_field);
}
//_________________________________________________________________________________________
double Recombination::BirksRecombination(double dEdx, double electric_field) {
    double ARecomb = 0.800;
    double kRecomb = 0.0486;

    return ARecomb / (1. + dEdx * kRecomb / electric_field);
}
//_________________________________________________________________________________________
double Recombination::EscapeRecombination(double dEdx, double electric_field) {
    ...
    double field_correction = std::exp(-electric_field  / (larqlAlpha * std::log(dEdx) + larqlBeta));
